...
typecheck:
	. .venv/bin/activate && mypy answers/src --explicit-package-bases && mypy practice/src --explicit-package-bases

qa: fmt lint typecheck

security:
	. .venv/bin/activate && bandit -q -r answers/src practice/src -x tests -s B101
	. .venv/bin/activate && pip-audit -r requirements.txt || true

ci:
	. .venv/bin/activate && black --check . && isort --check-only . && flake8 $(shell git ls-files '*.py' | tr '\n' ' ') && pylint -j 0 $(shell git ls-files '*.py' | tr '\n' ' ') && mypy answers/src --explicit-package-bases && mypy practice/src --explicit-package-bases && bandit -q -r answers/src practice/src -x tests -s B101
